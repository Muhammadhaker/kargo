...
function CargoList({ filteredItems, searchQuery, setSearchQuery, calculateCosts, handleEdit, handleDelete }) {
  return (
    <div className="lg:col-span-2 space-y-4">
      ...
      {/* QIDIRUV */}
      <div className="relative w-full">
        <span className="absolute inset-y-0 left-0 flex items-center pl-4"><RiSearchLine className="text-slate-500" /></span>
        <input 
          type="text" 
          value={searchQuery}
          onChange={e => setSearchQuery(e.target.value)}
          placeholder="Trek kod yozib qidiring..."
          className="w-full bg-slate-900 border border-slate-800 text-white rounded-xl pl-11 pr-4 py-3.5 focus:outline-none focus:border-amber-500 transition-all text-sm"
        />
      </div>

      <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest pt-2">
        📦 Yuklar Ombori ({filteredItems.length} ta)
      </h3>

      {/* MOBIL VA PLANSHT UCHUN KARTALAR (md dan kichik ekranlar) */}
      <div className="md:hidden space-y-3">
        {filteredItems.map((item) => (
          <div key={item._id} className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex gap-4">
            {item.imageUrl ? <img src={item.imageUrl} className="w-14 h-14 object-cover rounded-xl" /> : <div className="w-14 h-14 bg-slate-950 rounded-xl flex items-center justify-center text-[8px] text-slate-700">NO IMG</div>}
            <div className="flex-1 min-w-0">
              <h4 className="font-black text-white truncate">{item.trackingCode}</h4>
              <p className="text-[10px] text-slate-400">{item.weight} kg • {item.quantity} ta • {item.status}</p>
              <p className="text-emerald-400 font-bold text-xs mt-1">{calculateCosts(item).totalSom.toLocaleString()} so'm</p>
            </div>
            <div className="flex flex-col gap-2">
              <button onClick={() => handleEdit(item)} className="p-2 bg-slate-800 rounded-lg text-amber-400"><RiEdit2Line /></button>
              <button onClick={() => handleDelete(item._id)} className="p-2 bg-slate-800 rounded-lg text-red-400"><RiDeleteBin6Line /></button>
            </div>
          </div>
        ))}
      </div>

      {/* KOMPYUTER UCHUN JADVAL (md dan katta ekranlar) */}
      <div className="hidden md:block border border-slate-800 rounded-2xl bg-slate-900 overflow-hidden">
        <table className="w-full text-left text-xs">
          <thead>
            <tr className="bg-slate-950/60 text-slate-400 uppercase">
              <th className="py-3 px-4">Rasm</th>
              <th className="py-3 px-4">Trek</th>
              <th className="py-3 px-4">Holat</th>
              <th className="py-3 px-4">Vazn/Soni</th>
              <th className="py-3 px-4">Jami</th>
              <th className="py-3 px-4 text-center">Amal</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-800">
            {filteredItems.map((item) => (
              <tr key={item._id} className="hover:bg-slate-800/20">
                <td className="py-3 px-4">{item.imageUrl ? <img src={item.imageUrl} className="w-9 h-9 object-cover rounded-lg" /> : "—"}</td>
                <td className="py-3 px-4 font-bold text-white">{item.trackingCode}</td>
                <td className="py-3 px-4"><span className="bg-slate-800 px-2 py-1 rounded">{item.status}</span></td>
                <td className="py-3 px-4">{item.weight} kg / {item.quantity} ta</td>
                <td className="py-3 px-4 text-emerald-400 font-bold">{calculateCosts(item).totalSom.toLocaleString()} so'm</td>
                <td className="py-3 px-4 flex justify-center gap-2">
                  <button onClick={() => handleEdit(item)} className="text-amber-400"><RiEdit2Line /></button>
                  <button onClick={() => handleDelete(item._id)} className="text-red-400"><RiDeleteBin6Line /></button>
                </td>
              </tr>
            ))}
          </tbody>
