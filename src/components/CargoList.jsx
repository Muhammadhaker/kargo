...
function CargoList({ filteredItems, searchQuery, setSearchQuery, calculateCosts, handleEdit, handleDelete }) {
  ...
  return (
    <div className="lg:col-span-2 space-y-4 min-w-0">
        ...
        {filteredItems.length === 0 ? (
          ...
          filteredItems.map((item) => {
            ...
            return (
              <div
                key={item._id}
                onClick={() => setSelectedItem(item)} // 🚀 Modal ochilishi uchun
                className="bg-slate-900 border border-slate-800 rounded-2xl p-4 relative overflow-hidden flex flex-col gap-3 cursor-pointer hover:border-amber-500/30 transition-all"
              >
                ...
                {/* 🚀 stopPropagation() tugma bosilganda modal ham qo'shilib ochilib ketmasligi uchun */}
                <div className="absolute top-3 right-3 flex gap-1.5" onClick={e => e.stopPropagation()}>
                  <button onClick={() => handleEdit(item)} className="p-2 bg-slate-950 border border-slate-800 rounded-xl text-amber-400 hover:bg-amber-500/20 transition-all">
                    <RiEdit2Line className="text-xs" />
                  </button>
                  <button onClick={() => handleDelete(item._id)} className="p-2 bg-slate-950 border border-slate-800 rounded-xl text-red-400 hover:bg-red-500/20 transition-all">
              ...
              filteredItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-2 text-center pr-3" onClick={e => e.stopPropagation()}>
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setSelectedItem(null)}>
          <div className="bg-slate-900 w-full max-w-sm rounded-3xl border border-slate-700 overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6">
              <h3 className="text-xl font-black text-amber-500 text-center mb-5 break-all">{selectedItem.trackingCode}</h3>
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={() => { handleEdit(selectedItem); setSelectedItem(null); }} className="flex-1 bg-blue-600/10 hover:bg-blue-600/20 border border-blue-600/30 text-blue-400 py-3 rounded-xl font-bold transition-all">Tahrirlash</button>
                <button onClick={() => { handleDelete(selectedItem._id); setSelectedItem(null); }} className="flex-1 bg-red-600/10 hover:bg-red-600/20 border border-red-600/30 text-red-400 py-3 rounded-xl font-bold transition-all">O'chirish</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* 🔍 ZOOM OYNA */}
      {zoomedImg && (
        <div className="fixed inset-0 z-[60] bg-black/95 flex items-center justify-center p-4 cursor-zoom-out" onClick={() => setZoomedImg(null)}>
          <img src={zoomedImg} className="max-w-full max-h-full rounded-xl shadow-2xl border border-slate-800" />
          <button className="absolute top-6 right-6 text-white bg-white/10 hover:bg-red-500 rounded-full w-12 h-12 flex items-center justify-center text-xl transition-colors backdrop-blur-md">✕</button>
        </div>
      )}
    </div>
  );
